...
@Component({
  ...
  templateUrl: "./streaming-server.component.html",
  styleUrls: ["./streaming-server.component.scss"],
  encapsulation: ViewEncapsulation.None,
})
export class StreamingServerComponent implements OnInit {
  ...
  // Get streaming servr status
  getStatus() {
    this.systemServerSvc
      .getLiveStreamingServerStatus()
      .subscribe((res: any) => {
        this.getServerStatus(res.status);
      });
  }
  // show pie charts
  showPieCharts() {
    this.systemServerSvc
      .getServerData(typesOfServer.apiServer)
      .subscribe((res: any) => {
        if (res.data) {
  ...
  getServerStatus(data) {
    if (data != 200) return (this.statusData = { status: "In-active" });
    if (data == 200) return (this.statusData = { status: "Active" });
  }

  // Get Band width
  getBandWidth() {
    this.systemServerSvc
      .getServerBandWidth(typesOfServer.apiServer)
      .subscribe((res: any) => {
        if (res.data) {
